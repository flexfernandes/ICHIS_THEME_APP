/**
 * GF Desk Modern — desk_modern.js v26
 * GREENFARMS | ichis_theme_app
 *
 * MUDANÇAS v26:
 * - Tema segue EXCLUSIVAMENTE o Frappe nativo (localStorage desk_theme / frappe.ui.dark_mode)
 * - Removida dependência do campo tema_ativo do GF Theme Settings
 * - Avatar com foto real do usuário
 * - Dropdown espaçado, sem ícones, sem vermelho
 * - Alternância instantânea sincronizada com Frappe nativo
 * - Tarja preta ocultada via CSS
 * - Avatar com margem adequada
 */
(function () {
  "use strict";
  ...
  // ── Módulos ──────────────────────────────────────────────────
  var GF_MODULES = [
    { title:"Vendas",        desc:"Pedidos, clientes e faturamento",    icon:"🛒", accent:"#276749", route:"selling"      },
    { title:"Compras",       desc:"Fornecedores e recebimentos",        icon:"📦", accent:"#2563eb", route:"buying"       },
    { title:"Estoque",       desc:"Produtos, armazéns e movimentações", icon:"🏭", accent:"#b45309", route:"stock"        },
    { title:"Financeiro",    desc:"Contas, pagamentos e conciliação",   icon:"💰", accent:"#be185d", route:"accounts"     },
    { title:"RH",            desc:"Colaboradores, folha e benefícios",  icon:"👥", accent:"#0369a1", route:"hr"           },
    { title:"Projetos",      desc:"Tarefas, cronogramas e equipes",     icon:"📋", accent:"#7c3aed", route:"project"      },
    { title:"CRM",           desc:"Leads, oportunidades e contatos",    icon:"🤝", accent:"#c2410c", route:"crm"          },
    { title:"Manufatura",    desc:"Ordens de produção e BOM",           icon:"⚙️", accent:"#374151", route:"manufacturing" },
    { title:"Qualidade",     desc:"Controle e inspeções",               icon:"✅", accent:"#065f46", route:"quality"      },
    { title:"Relatórios",    desc:"Análises e relatórios gerenciais",   icon:"📊", accent:"#15803d", route:"query-report" },
    { title:"Configurações", desc:"Administração do sistema",           icon:"🔧", accent:"#374151", route:"setup"        },
  ];
  ...
  // ── Detecta dark mode pelo Frappe nativo ─────────────────────
  // Frappe v16 usa localStorage["desk_theme"] = "dark" | "light"
  // e frappe.ui.dark_mode (boolean)
  function _isDarkMode() {
    try {
      // 1. Frappe nativo — fonte primária de verdade
      var frappeNative = "";
      try { frappeNative = localStorage.getItem("desk_theme") || ""; } catch(e){}
      if (frappeNative === "dark") return true;
      if (frappeNative === "light") return false;

      // 2. frappe.ui.dark_mode (boolean)
      if (typeof frappe !== "undefined" && frappe.ui &&
          typeof frappe.ui.dark_mode === "boolean") {
        return frappe.ui.dark_mode;
      }

      // 3. data-bs-theme no html/body (Frappe v16 Bootstrap 5)
      ...
      if (document.body.classList.contains("dark-mode") ||
          document.body.classList.contains("frappe-dark")) return true;

      return false;
    } catch(e) { return false; }
  }

  // ── Aplica dark mode no container ───────────────────────────
  function _applyTheme() {
    if (!_container) return;
    var dark = _isDarkMode();
    if (dark) {
      _container.classList.add("gf-dark-mode");
    } else {
      _container.classList.remove("gf-dark-mode");
    }
    // Atualiza label do botão
    var lbl = document.getElementById("gf-theme-label");
    if (lbl) lbl.textContent = dark ? "Alternar para Light" : "Alternar para Dark";
  }
  ...
  function _show() {
    ...
    // Avatar — foto real do usuário
    var userImage = user.image || boot.user_image || "";
    // Tenta também o caminho padrão do Frappe
    if (!userImage && frappe && frappe.session && frappe.session.user) {
      var usr = frappe.session.user;
      var ui = (boot.user_info && boot.user_info[usr]) || {};
      userImage = ui.image || "";
    }
    ...
    _container.className = "gf-modern-desk";

    // Aplica tema imediatamente (antes de inserir no DOM)
    if (_isDarkMode()) _container.classList.add("gf-dark-mode");
    ...
    var avatarInner = userImage
      ? '<img id="gf-user-img" src="' + userImage
          + '" style="width:34px;height:34px;border-radius:50%;object-fit:cover;">'
          + '<span id="gf-user-initial" class="gf-user-avatar" style="display:none">'
          + initial + '</span>'
        ...
        + '<div class="gf-topbar-right">'
          + '<div class="gf-user-menu" id="gf-user-menu">'
            ...
            + '<div class="gf-user-dropdown" id="gf-user-dropdown">'
              ...
              + '<button class="gf-user-dropdown-item" id="gf-action-theme">'
                  + '<span id="gf-theme-label">Alternar para Dark</span>'
                + '</button>'
              + '<button class="gf-user-dropdown-item" id="gf-action-theme-settings">Configurações do Tema</button>'
              + '<div class="gf-dropdown-sep"></div>'
    ...
    // Aplica tema após inserção (garante que variáveis CSS estejam disponíveis)
    _applyTheme();

    // Fallback do avatar: se imagem falhar, mostra inicial
    if (userImage) {
      var img = document.getElementById("gf-user-img");
      var ini = document.getElementById("gf-user-initial");
      if (img) {
    ...
    if (userBtn && dropdown) {
      userBtn.addEventListener("click", function(e) {
        ...
        if (!open) _applyTheme(); // atualiza label ao abrir
      });
      ...
      document.getElementById("gf-action-theme").addEventListener("click", function() {
        ...
        // Usa o mecanismo nativo do Frappe para alternar tema
        var nowDark = _isDarkMode();
    ...
    // Observa mudanças de tema feitas externamente (ex: botão nativo do Frappe)
    try {
      var themeObserver = new MutationObserver(function() { _applyTheme(); });
      themeObserver.observe(document.documentElement, {
        attributes: true, attributeFilter: ["data-bs-theme", "class"]
      });
      themeObserver.observe(document.body, {
        attributes: true, attributeFilter: ["data-bs-theme", "class", "data-gf-tema"]
      });
    } catch(e){}

    console.log("[GF Desk Modern] Ativo. Empresa:", company, "| Dark:", _isDarkMode());
  }
